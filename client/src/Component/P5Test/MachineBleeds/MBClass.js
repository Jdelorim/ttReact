...
export default class MBClass {
    ...
    constructor(p) {
        ...
        // this.ran1 = p.random(80,120);
        // this.ran2 = p.random(80,120);
        // this.ran3 = p.random(80,120);
        this.y = [];
        this.time = 0;
        this.amount = 20;
    ...
    display = (time) => {
        ...
        let {width, height, map, mouseX, sin} = this.p;
        let dist = 200;
        let x = width * .25;
        let speed = 0.01;
        for (let i =0;i<this.amount; i++){
        let p = map(i, 0,this.amount,0, height+100 );
        ...
        let n2x = map(this.simplex.noise2D(time*speed+this.toffset[i],this.r3), -1,1,-dist,dist);
        ...
        let n2y = map(this.simplex.noise2D(time*speed+this.toffset[i],this.r2), -1,1,-dist,dist);
        let n3y = map(this.simplex.noise2D(time*speed+this.toffset[i],this.r3), -1,1,-dist,dist);
        this.p.stroke(c,50, 100);
